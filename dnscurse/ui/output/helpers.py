...
def _colorize_domain(
    domain: str,
    zone: str | None,
    parent_zone: str | None = None,
    focus_color: str = _YELLOW,
    color: bool = True,
) -> str:
    ...
    if parent_plain and zone_lower_part.endswith(parent_plain):
        new_len = len(zone_part) - len(parent_plain) - 1
        # that would be -1 for dot separator
        ...
        focus, resolved = (
            (zone_part, "")
            if dot == -1
            else (zone_part[:dot], zone_part[dot:])
        )
...
def _format_result_line(step: models.RecursionStep) -> str:
    ...
    if resp.answer:
        cname_target = models.get_cname_target(resp, step.query_name)
        if cname_target:
    ...
    if models.is_referral(resp):
        ns_servers = models.get_referral_ns_servers(resp)
        if ns_servers:
            parts = (
                [f"{name.rstrip('.')} ({ip} glue)"
                for name, ip in ns_servers]
            )
            return "referral \u2192 " + ", ".join(parts)
        ns_names = models.get_referral_ns_names(resp)
        if ns_names:
...
def _format_tree(steps: list[models.RecursionStep], color: bool = True) -> str:
    ...
    # its IP comes from ROOT_SERVERS, not glue.
    # Node N's zone label is what node N-1 referred to
    # glue_ip is non-empty when
    ...
    for prev, step in zip(steps, steps[1:]):
        zone = models.get_delegated_zone(prev) or "?"
        glue = (
            {ip for _, ip in models.get_referral_ns_servers(prev.response)}
            if prev.response
            else set()
        )
        glue_ip = (
            step.server_ip
            if step.server_ip in glue
            else ""
        )
    ...
    for depth, (zone, _, _glue) in enumerate(nodes):
        ...
        parent_label_count = (
            0 if parent_plain in (".", "")
            else len(parent_plain.split("."))
        )
    ...
    for depth, (zone, server, glue_ip) in enumerate(nodes):
        ...
        if glue_ip:
            ...
            ip_str = ""
        lines.append(
            f"{indent}{connector}{_color_zone(zone, depth)}"
            f"({server}{ip_str})"
        )
    ...
    last_zone_label_count = (
        0
        if last_zone_plain in (".", "")
        else len(last_zone_plain.split("."))
    )
    new_label_count = n - last_zone_label_count
    leaf_new_labels = (
        ".".join(query_labels[:new_label_count])
        if new_label_count > 0
        else ""
    )
    leaf_label_str = (
        f"{lc}{leaf_new_labels}{lr} "
        if (color and leaf_new_labels)
        else ""
    )
...
def _format_step_block(
    ...
    """
    zone = models.get_delegated_zone(step)
